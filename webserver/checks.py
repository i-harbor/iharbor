...
def check_ceph_settins(app_configs, **kwargs):
    ...
    if not cephs:
        errors.append(Error('未配置CEPH集群信息，配置文件中配置“CEPH_RADOS”', hint='CEPH_RADOS'))

    if 'default' not in cephs:
        errors.append(Error('配置文件中CEPH集群信息配置“CEPH_RADOS”中必须存在一个别名“default”', hint='CEPH_RADOS.default'))
