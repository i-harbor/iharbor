...
class S3V4Authentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if 'x-amz-tagging' in request.headers:
            raise exceptions.AuthenticationFailed('Object tagging is not supported.')
    ...
    def get_credentials_from_header(self, request):
        ...
        if len(auth) == 1:
            raise exceptions.AuthenticationFailed("This request does not support credentials.")

        try:
            auth_key_str = auth[1].decode()
        except UnicodeError:
            msg = _('The provided security credentials are not valid.'
                    'Auth string should not contain invalid characters.')
    ...
    def get_credentials_from_query(self, request):
        ...
        if algorithm != 'AWS4-HMAC-SHA256':
            msg = 'The provided security credentials are not valid.' \
                  'The query param "X-Amz-Algorithm" must be "AWS4-HMAC-SHA256"'
            raise exceptions.AuthenticationFailed(msg)

        self.s3_timestamp = self.get_timestamp_from_query(request)
        if not self.s3_timestamp:
            msg = _('The provided security credentials are not valid.No query param "X-Amz-Date" provided.')
            raise exceptions.AuthenticationFailed(msg)
        ...
        if not (1 <= expires <= 86400):
            raise exceptions.AuthenticationFailed(
                _('The provided security credentials are not valid.invalid value of param "Expires".'))

        now_tsp = datetime.utcnow().timestamp()
        expires_tsp = self.s3_datetime.timestamp() + expires
        if expires_tsp < now_tsp:
            raise exceptions.AuthenticationFailed(_('The provided security credentials are not valid. expires'))

        credential = request.query_params.get('X-Amz-Credential', '')
        signature = request.query_params.get('X-Amz-Signature', '')
        if not credential:
            raise exceptions.AuthenticationFailed(
                'The provided security credentials are not valid.invalid value of query param "X-Amz-Credential".')

        if not signature:
            raise exceptions.AuthenticationFailed(
                'The provided security credentials are not valid.invalid value of query param "X-Amz-Signature".')
    ...
    def authenticate_credentials(self, request, credentials):
        ...
        if len(l_credential) < 4:
            raise exceptions.AuthenticationFailed(
                'The provided security credentials are not valid. invalid format "Credential"')
        ...
        try:
            ...
            raise exceptions.AuthenticationFailed('The access key ID you provided does not exist in our records.')
        ...
        if sig != signature:
            raise exceptions.AuthenticationFailed(
                ...
                "Check your secret access key and signing method." + f'{sig} != {signature}')
    ...
    def parse_auth_key_string(auth_key):
        ...
        if len(auth) != 3:
            raise exceptions.AuthenticationFailed(
                'The provided security credentials are not valid.length is not 3 split by ","')
        ...
        for a in auth:
            ...
            if name not in ['Credential', 'SignedHeaders', 'Signature']:
                raise exceptions.AuthenticationFailed(
                    ...
                    'key must be in ("Credential", "SignedHeaders", "Signature")')
            ret[name] = val
    ...
    def generate_signature(self, request, signed_headers: str, secret_key: str):
        try:
            ...
            raise exceptions.APIException(f'An error occurred while calculating the signature, {str(e)}')
    ...
    def canonical_headers(self, request, signed_headers: str):
        ...
        for key in sorted_header_names:
            ...
            if not v:
                raise exceptions.AuthenticationFailed(
                    f'The provided security credentials are not valid.The header "{key}" was not provided.')
