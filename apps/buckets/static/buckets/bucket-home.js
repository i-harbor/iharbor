;(function() {
    ...
    let render_bucket_item = template.compile(`
        <tr class="active" id="bucket-list-item">
            ...
            <td><span class="glyphicon glyphicon-oil"></span><span>  </span><a>{{ $data['name'] }}</a></td>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-xs-12 col-sm-12">
                    ...
                    <div id="new-bucket-div" hidden>
                        <form id="form-new-bucket" method="POST" action="<%= action_url %>" enctype="multipart/form-data">                 
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12">
                    <table class="table table-hover" id="bucket-table">
                        ...
                        {{if buckets}}
                            {{ each buckets }}
                                <tr class="active" id="bucket-list-item">
                                    ...
                                    <td><span class="glyphicon glyphicon-oil"></span><span>  </span><a id="bucket-list-item-enter" bucket_name="{{ $value.name }}">{{ $value.name }}</a>
                                    </td>
                     ...
                     {{if buckets.length === 0}}
                            <p class="text-info text_center">空空如也</p>
                      {{/if}}
                </div>
            </div>
        </div>
    ...
    function get_buckets_and_render(){
        ...
        $.ajax({
            ...
            success: function(data,status,xhr){
                swal.close();
                if (status === 'success'){
                    let html = render_bucket_view(data);
                    $content_display_div.append(html);
                }else{
                    $content_display_div.append('<p class="text_center text-info">好像哪里出问题了，跑丢了</p>');
                }
            },
                            ...
                            <tr class="bucket-files-table-item">
                                ...
                                {{ if $value.fod }}
                                    <td class="bucket-files-table-item">
                                        ...
                                        <a id="bucket-files-item-enter-file">{{ $value.na }}</a>
                                    </td>
                                    <td>{{ $value.ult }}</td>
                                    <td>{{ $value.si }}</td>
                                {{/if}}
                                {{ if !$value.fod }}
                                    <td>
                                        ...
                                        <a id="bucket-files-item-enter-dir" dir_path="{{$value.na}}"><strong class="bucket-files-table-item" >{{ $value.na }}</strong></a>
                                    </td>
                                    <td>{{ $value.ult }}</td>
                                    <td>--</td>
                                {{/if}}
    ...
    $("#content-display-div").on("click", '#bucket-files-item-enter-dir', function (e) {
        ...
        console.log(bucket_name, dir_path);
    ...
    $("#content-display-div").on("click", '#bucket-files-item-enter-file', function (e) {
        e.preventDefault();
        // get_bucket_files_and_render(bucket_name='wang');
    });
    ...
    function get_bucket_files_and_render(bucket_name, dir_path=''){
        ...
        $.ajax({
            ...
            success: function(data,status,xhr){
                swal.close();
                if (status === 'success'){
                    let html = render_bucket_files_view(data);
                    $content_display_div.append(html);
                }else{
                    $content_display_div.append('<p class="text_center text-info">好像哪里出问题了，跑丢了</p>');
                }
    ...
    // 获取文件对象信息并渲染
    //
    function get_file_obj_info_and_render(bucket_name='wang'){
        $content_display_div.empty();
        swal.showLoading();
        $.ajax({
            url: 'api/v1/directory/',
    ...
    let render_bucket_dir_item = template.compile(`
        <tr class="bucket-files-table-item">
            ...
            <td>
                <span class="glyphicon glyphicon-folder-open"></span>
                <a><strong class="bucket-files-table-item" id="bucket-files-item-enter-dir">{{ dir.na }}</strong></a>
            </td>
            <td>{{ dir.ult }}</td>
            <td>--</td>
            <td>
